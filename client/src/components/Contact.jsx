...
const Contact = () => {
  ...
  const handleComment = async (e) => {
    ...
    let res = await fetch("http://localhost:5000/api/auth/comment", {
      method: "POST",
      ...
      <div className="container px-5 py-24 mx-auto flex">
        ...
        <div className="lg:w-1/3 md:w-1/2 bg-gray-400 bg-opacity-40 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
          ...
          <div className="relative mb-4">
            ...
            <input type="email" id="email" name="email" onChange={handleInput} className="w-full bg-white rounded border border-gray-300 focus:border-red-500 focus:ring-2 focus:ring-red-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
          </div>
          <div className="relative mb-4">
            <label htmlFor="message" className="leading-7 text-lg text-gray-600 font-bold">Message</label>
            <textarea id="comment" name="comment" onChange={handleInput} className="w-full bg-white rounded border border-gray-300 focus:border-red-500 focus:ring-2 focus:ring-red-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></textarea>
          </div>
